...
_BACKEND_COMMAND_PATTERNS: Final[tuple[tuple[re.Pattern[str], str], ...]] = (
    (re.compile(r"uvicorn\s+[\w.:]+", re.I), "uvicorn"),
    (re.compile(r"flask\s+run", re.I), "flask run"),
    (re.compile(r"gunicorn\s+", re.I), "gunicorn"),
    (re.compile(r"python\s+manage\.py\s+runserver", re.I), "python manage.py runserver"),
...
_DEFAULT_READY_TIMEOUT: Final[int] = 120
...
class EnvironmentManager:
    ...
    def detect_stack(self, local_repo_path: str) -> dict[str, Any]:
        ...
        Returns:
            Dictionary with ``db_type``, ``backend_start_command``,
            ``frontend_start_command``, ``frontend_port``, and
            ``required_env_vars`` keys. Missing values are ``None`` or empty lists.
        """
        ...
        try:
            db_type = self._detect_db_type(root)
            backend_command = self._detect_backend_command(root)
            ...
            raise EnvironmentError(f"detect_stack failed for {root}: {exc}") from exc
    ...
    def start_test_database(self, db_type: str) -> str:
        ...
        Raises:
            EnvironmentError: If Docker is unavailable or the DB fails to start.
        """
    ...
    def run_migrations(self, local_repo_path: str, connection_string: str) -> bool:
        ...
        try:
            ...
            raise EnvironmentError(f"run_migrations failed for {root}: {exc}") from exc
    ...
    def stop_test_database(self) -> None:
        ...
        if self._docker_container_id:
            ...
            try:
                ...
                raise EnvironmentError(
                    f"stop_test_database failed while stopping container {container_id}: {exc}"
                ) from exc
        ...
        if self._sqlite_path and self._sqlite_path.exists():
            try:
                ...
                raise EnvironmentError(
                    f"stop_test_database failed while deleting {self._sqlite_path}: {exc}"
                ) from exc
            finally:
                self._sqlite_path = None

    def _detect_db_type(self, root: Path) -> str | None:
    ...
    def _detect_backend_command(self, root: Path) -> str | None:
        ...
        for path in self._iter_source_files(root):
            try:
                ...
                continue
            for pattern, command in _BACKEND_COMMAND_PATTERNS:
    ...
    def _start_docker_database(
        ...
        *,
        ...
        try:
            ...
            raise EnvironmentError(
                f"start_test_database failed while running docker: {exc.stderr or exc.stdout or exc}"
            ) from exc
        except Exception as exc:
            self.stop_test_database()
            raise EnvironmentError(f"start_test_database failed for image={image}: {exc}") from exc
